// src: https://github.com/mauidude/go-readability/blob/2f30b1a346f19ddab94ffe0cb788331be9399de2/readability.go
...
func NewDocument(s []byte, url *url.URL) (*Document, error) {
	d := &Document{
		input:                    s,
		url:                      url,
...
func (d *Document) initializeHtml(s []byte) error {
	// replace consecutive <br>'s with p tags
	s = replaceBrsRegexp.ReplaceAll(s, []byte("</p><p>"))

	// replace font tags
	s = replaceFontsRegexp.ReplaceAll(s, []byte(`<${1}span>`))

	// manually strip regexps since html parser seems to miss some
	s = stripCommentRegexp.ReplaceAll(s, []byte{})
...
func (d *Document) IconURL() []string {
	if d.iconURLs == nil {
		d.document.Find("head").Each(func(i int, s *goquery.Selection) {
			d.document.Find("link").Each(func(i int, s *goquery.Selection) {
				href, exists := s.Attr("href")
				if !exists {
					return
				}

				extention := filepath.Ext(href)
				...
				if u.Hostname() != "" {
					d.iconURLs = append(d.iconURLs, u.String())
					return
				}

				d.iconURLs = append(d.iconURLs, d.url.String()+href)
			})
		})
	}
	return d.iconURLs
...
func (d *Document) Title() string {
	if d.title == "" {
		d.document.Find("title").EachWithBreak(func(i int, s *goquery.Selection) bool {
			d.title = s.Text()
			return false
		})
	}
...
func (d *Document) Content() []byte {
	if len(d.content) == 0 {
		...
		length := len(bytes.TrimSpace(articleText))
...
func (d *Document) getArticle() []byte {
...
func (d *Document) sanitize(article []byte) []byte {
	doc, err := goquery.NewDocumentFromReader(bytes.NewReader(article))
	if err != nil {
	...
	return normalizeWhitespaceRegexp.ReplaceAll(text, []byte("\n"))
}
