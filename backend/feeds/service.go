...
func (s *Service) List(ctx context.Context, userID string, pageSize int, createdLT *time.Time, tagIDIn []string) ([]*Feed, error) {
	feeds, err := s.db.List(ctx, userID, pageSize, createdLT, tagIDIn)
	switch {
	case err == nil:
		return feeds, nil
	default:
